...
var officerStatusController = {
  ...
  getOfficerStatus: async (req, res, next) => {
    try {
      const { id } = req.params;
      list = id.split(".");
      const result = await officerStatusService.getOfficerStatus(list[list.length - 1]);
      return res.status(result.status).json(result);
    } catch (error) {
      return next(error);
    }
  ...
  putOfficerStatus: async (req, res, next) => {
    try {
      ...
      const result = await officerStatusService.putOfficerStatus(list[list.length - 1], {
        name,
        description,
        color,
      });
