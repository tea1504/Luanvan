...
var securityService = {
  ...
  getSecurities: async (limit = 10, pageNumber = 1, filter = "") => {
    try {
      const result = {};
      const totalSecurity = await securityModel.countDocuments({ deleted: false });
  ...
  getSecurity: async (id) => {
    try {
      const type = await securityModel.findOne({ _id: id, deleted: false });
      if (!type)
        return {
          status: Constants.ApiCode.NOT_FOUND,
          message: Constants.String.Message.ERR_404(Constants.String.Security._),
        };
      return {
        status: Constants.ApiCode.SUCCESS,
        message: Constants.String.Message.GET_200(Constants.String.Security._),
  ...
  putSecurity: async (id, security) => {
    try {
      const findSecurity = await securityModel.findOne({ _id: id, deleted: false });
      if (!findSecurity)
        return {
          status: Constants.ApiCode.NOT_FOUND,
          message: Constants.String.Message.ERR_404(Constants.String.Security._),
      ...
      await securityModel.findOneAndUpdate({ _id: id, deleted: false }, security);
  ...
  deleteSecurity: async (id) => {
    try {
      ...
      return {
        status: Constants.ApiCode.SUCCESS,
        message: Constants.String.Message.DELETE_200(Constants.String.Security._),
