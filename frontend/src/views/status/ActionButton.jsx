...
import { setData, setTotal } from 'src/store/slice/status.slide'
      ...
      {loggedUser.right[Strings.Common.READ_CATEGORIES] && (
        <CTooltip content={Strings.Common.DETAIL}>
          <CButton
            ...
            className="m-1"
            onClick={() =>
              navigate(Screens.STATUS_DETAIL(`${Helpers.toSlug(data.name)}.${data._id}`))
            }
          >
      ...
      {loggedUser.right[Strings.Common.UPDATE_CATEGORIES] && (
        <CTooltip content={Strings.Common.EDIT}>
          <CButton
            ...
            className="m-1"
            onClick={() =>
              navigate(Screens.STATUS_UPDATE(`${Helpers.toSlug(data.name)}.${data._id}`))
            }
          >
