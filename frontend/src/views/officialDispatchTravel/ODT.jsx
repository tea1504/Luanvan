...
import { setData, setPage, setRowPerPage, setTotal } from 'src/store/slice/ODT.slide'
import Swal from 'sweetalert2'
import withReactContent from 'sweetalert2-react-content'
import IODColumns from './ODTColumns'
...
import { ODTToPrint } from './ODTToPrint'
import { useRef } from 'react'
import OfficerService from 'src/services/officer.service'
import ODTColumns from './ODTColumns'
...
export default function ODT() {
  ...
  const pathname = func === Screens.ODT_LIST ? Screens.ODT_LIST : Screens.ODT_LIST_(func)
  ...
  const handleOnChangeSelectType = (selectedItem) => {
    ...
    if (findParams.code) findParamsTemp.code = findParams.code
    if (selectedItem) {
  ...
  const handleOnChangeCode = (e) => {
    ...
    if (findParams.status) findParamsTemp.status = findParams.status
    if (e.target.value) {
      setFindParams({ code: e.target.value, ...findParamsTemp })
      navigate({
        pathname: pathname,
        search: `?${createSearchParams({
          page: store.page,
  ...
  const handleOnClickButtonDeleteCode = () => {
    updateFindParams({ code: '' })
    delete findParams.code
    navigate({
      pathname: pathname,
      search: `?${createSearchParams({
        page: store.page,
  ...
  const handleOnClickSearchButton = async () => {
    try {
      ...
      delete findParams.code
      if (!findParams.codeStart) delete findParams.codeStart
      if (!findParams.codeEnd) delete findParams.codeEnd
  ...
  const handleOnChangeArriveNumberStartInput = (e) => {
    if (findParams.codeEnd < parseInt(e.target.value))
      updateFindParams({
        codeStart: parseInt(e.target.value),
        codeEnd: parseInt(e.target.value),
      })
    else
      updateFindParams({
        codeStart: parseInt(e.target.value),
  ...
  const handleOnChangeArriveNumberEndInput = (e) => {
    if (findParams.codeStart > parseInt(e.target.value))
      updateFindParams({
        codeStart: parseInt(e.target.value),
        codeEnd: parseInt(e.target.value),
      })
  ...
  const handleOnClickDeleteCodeRange = () => {
    updateFindParams({
      codeStart: '',
      codeEnd: '',
    })
  ...
  useEffect(() => {
    ...
    const codeStart = searchParams.get('codeStart') || ''
    const codeEnd = searchParams.get('codeEnd') || ''
    ...
    if (code) findParamsObject.code = code
    if (codeStart) findParamsObject.codeStart = codeStart
    if (codeEnd) findParamsObject.codeEnd = codeEnd
        ...
        <CCol>
          <CCard className="mb-3 border-secondary border-top-5">
            <CCardHeader className="text-center py-3" component="h3">
              {func === Screens.ODT_LIST && Strings.OfficialDispatchTravel.NAME}
              {func === Screens.ODT_APPROVAL && Strings.OfficialDispatchTravel.Title.LIST_APPROVAL}
              {func === Screens.ODT_HANDLE && Strings.OfficialDispatchTravel.Title.LIST_HANDLE}
              {func === Screens.ODT_IMPLEMENT &&
                Strings.OfficialDispatchTravel.Title.LIST_IMPLEMENT}
              {func === Screens.ODT_REFUSE && Strings.OfficialDispatchTravel.Title.LIST_REFUSE}
              {func === Screens.ODT_LATE && Strings.OfficialDispatchTravel.Title.LIST_LATE}
            </CCardHeader>
            <CCardBody>
              <CRow className="py-1">
                ...
                <CCol xs={12} md={6} className="text-end mt-1">
                  <CButtonGroup role="group">
                    ...
                    {loggedUser?.right[Strings.Common.CREATE_OD] && func === Screens.ODT_LIST && (
                      <CButton
                        color="primary"
                        variant="outline"
                        onClick={() => navigate(Screens.ODT_CREATE)}
                      >
                ...
                <div className="d-none">
                  <ODTToPrint ref={printRef} data={store.data} />
                </div>
              </CRow>
              <CRow
                xs={{ cols: 1 }}
                md={{ cols: func === Screens.ODT_LIST ? 3 : 2 }}
                className="pb-1"
              >
                <CCol className="mt-1">
                  <Select
                ...
                <CCol className="mt-1">
                  <CInputGroup className="flex-nowrap">
                    <CFormInput
                      type="number"
                      placeholder={Strings.Form.FieldName.CODE()}
                      value={findParams.code}
                      onChange={handleOnChangeCode}
                    />
                    <CInputGroupText
                      id="addon-wrapping"
                      style={{ cursor: 'pointer' }}
                      onClick={handleOnClickButtonDeleteCode}
                    >
                      <FaEraser />
                    </CInputGroupText>
                  </CInputGroup>
        ...
        <CModalBody>
          <CRow className="mt-2">
            ...
            <CCol>
              <DatePicker
                id={Helpers.makeID(
                  Strings.IncomingOfficialDispatch.CODE,
                  Helpers.propName(String, Strings.Form.FieldName.ISSUED_DATE),
                )}
                ...
                maxDate={new Date()}
                isClearable
                placeholderText={Strings.Form.FieldName.ISSUED_DATE()}
                autoComplete="off"
              />
            </CCol>
          </CRow>
          <CRow className="mt-2">
            ...
            <CCol xs={12} sm={4}>
              <CFormInput
                type="number"
                placeholder={Strings.OfficialDispatchTravel.Common.CODE_START}
                min={0}
                value={findParams.codeStart}
            ...
            <CCol xs={12} sm={4}>
              <CFormInput
                type="number"
                placeholder={Strings.OfficialDispatchTravel.Common.CODE_END}
                min={0}
                value={findParams.codeEnd || findParams.codeStart}
                onChange={handleOnChangeArriveNumberEndInput}
              />
            </CCol>
            <CCol xs={12} sm={1}>
              <CButton className="w-100" color="danger" onClick={handleOnClickDeleteCodeRange}>
          ...
          {(func === Screens.ODT_LIST || func === Screens.ODT_LIST_(Screens.HANDLE)) && (
            <CRow className="mt-2">
              <CCol xs={12} sm={2}>
                <CFormLabel
                  htmlFor={Helpers.makeID(
